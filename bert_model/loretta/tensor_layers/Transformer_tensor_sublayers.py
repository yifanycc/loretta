...
class Transformer_classifier(nn.Module):
    def __init__(self,config):
        ...
        self.fc2 = wrapped_linear_layers(config.d_model,config.num_class, tensorized=False,bias=True)
    ...
    def forward(self,input,config_forward=None):
        output = self.fc1(input,config_forward=config_forward)
        output = self.fc2(self.dropout(self.act(output)))
...
class Transformer_Embedding(nn.Module):
    def __init__(self,config):
        ...
        if not hasattr(config,'emb_tensorized'):
            config.emb_tensorized = config.tensorized
        if config.emb_tensorized==True:
        ...
        self.tensorized = config.emb_tensorized
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.slf_attn = Tensor_Attention(config)
        self.pos_ffn = Tensor_PFF(config)
